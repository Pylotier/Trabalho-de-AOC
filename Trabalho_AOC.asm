...
verificar: # Verifica se o valor segue o parametro
	...
	li $s0, 1 # Define um conatador como 1 para começar a dividir no 'acharPrimo'
	j acharPrimo
menorQue1:
	li $v0, 4
	la $a0, msgMenor
...
acharPrimo:
	rem $s1, $t2, $s0 # Faz o resto do valor atual com um contador($s0)
	beq $s1, 0, nDivisores # Pergunta se o resto é igual a 0
	add $s0, $s0, 1 # Se a divisão não for 0, então soma o contador($s0) +1 para a proxima divisão
	bne $s0, $t2, acharPrimo # Continua o loop até que contador($s0) seja igual a $t2
	j continuar
nDivisores:
	add $t7, $t7, 1 # Contador de divosores do número atual
	add $s0, $s0, 1 # Soma contador($s0) +1 para continuar o fluxo
	bgt $t7, 1, naoPrimo # Inica novamente o loop, se o número de divisores for maior do que 2 vai para 'naoPrimo'
	j acharPrimo
continuar:
